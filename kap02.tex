...
\begin{definice}[Speciální cesty]
    ...
    % která prochází každým vrcholem maximálně jednou s výjimkou, že může začínat i končit ve stejném vrcholu.
    % se každý prvek vyskytuje nejvýše jednou. V případě redukované neorientované cesty nepovolujeme dva prvky, které se liší pouze obrácením vrcholů.
\end{definice}
\begin{pozn}\label{pozn-redukce-cesty}
    Z libovolné cesty $P$ z vrcholu $A$ vytvoříme redukovanou tak, že postupně volíme hranu $e$, která má v cestě dva \uv{neorientované} výskyty a vymažeme všechny hrany mezi nimi. Zároveň odstraníme jeden nebo oba \uv{neorientované} výskyty $e$, aby cesta navazovala. Jeden v případě, že prvky mají v cestě stejný směr, oba v případě, že prvky mají opačný směr.
    % vrchol, přes který cesta vede opakovaně a odstraníme všechny hrany mezi těmito opakováními. 
    % mezi hranami s prvním a posledním výskytem vrcholu $C$ dostaneme uzavřenou redukovanou cestu z vrcholu $A$.
    % vezmeme první vrchol $ (,A,q)$ v cestě $P$ (kromě vrcholu $A$ v první hraně v případě cyklu), který se v této cestě vyskytuje dvakrát. 
...
\end{definice}
V cyklu se tedy nemohou opakovat \uv{neorientované} hrany, může ale procházet stejným vrcholem vícekrát.
...
% , a na základě kterých volí následující tah.
...
Strom [n,k]-Mastermindu má nekonečnou množinu vrcholů. Každý vrchol je ale konečným stavem. Zároveň z definice hran plyne, že Strom [n,k]-Mastermindu je kořenový strom, což ukážeme v následujícím pozorování.
    % Do každého vrcholu (kromě počátečního) navíc vede právě jedna hrana ze stavu, který vznikne vynecháním posledního kola. Díky tomu můžeme říct, že strom [n,k]-Mastermindu je kořenový.
...
Nyní ukážeme nějaké vlastnosti multigrafu [n,k]-Mastermindu, které pomohou ukázat jeho vztah se stromem [n,k]-Mastermindu. Následující vlastnost grafu byla inspirovaná na webu \cite{DAG}.
...
\end{definice}
\begin{pozn}
    Tato vlastnost tedy povoluje hrany z vrcholu do sebe samotného. Ve~chvíli, kdy ale hrana opustí vrchol, už se do něj po orientovaných hranách nelze vrátit. Tuto vlastnost dokážeme pro multigraf [n,k]-Mastermindu.
    % To připomíná vlastnost kořenového stromu až na existenci neorientovaných cyklů.
\end{pozn}
