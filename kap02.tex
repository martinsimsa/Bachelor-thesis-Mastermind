...
%Pro každý stav lze nalézt množinu kódů, které mají s pokusy ve stavu stejné ohodnocení. 
...
%\begin{pozn} Platí následující pozorování.\[K_{u,r} = \{w \in K \mid s(u,w) = r\}.\] \end{pozn}
...
Průběh [n,k]-Mastermindu s nějakým tajným kódem $v$ lze sledovat ve stromu [n,k]-Mastermindu. Nechť $A$ je stav a hráč zvolí další pokus $u$. Touto volbou hráč vybral další vrstvu stavů hry, kterou jsou právě potomci stavu $A$ vzhledem ke kódu $u$. Zadavatel ohodnotí kód $u$ vzhledem ke kódu $v$ ohodnocením $r$. Ohodnocením určí, který stav z potomků $A$ vzhledem ke kódu $u$ bude následovat. Hráč tedy může vybrat následující pokus ten, který bude mít nejvhodnější vrstvu potomků vzhledem ke stavu $A$. Z těchto stavů ale neví, do kterého se hra dostane, protože nezná ohodnocení s tajným kódem. 

% Kdybychom si zapisovali uvedený strom pro všechny tajné kódy $v \in H_{n,k}$, tedy bychom se mohli do všech následujících neprázdných stavů, pomocí různých tajných kódů.  sestrojili bychom takzvaný strom algoritmu. 
...
% Zajímají nás všichni potomci až do chvíle, kdy dospějeme do konečného stavu, který odpovídá dohrané hře. 

% Zadavatel ohodnocením kódu ... Toto ohodnocení následně určí zadavatel. ...Následně zahraný kód s ohodnocením jednoznačně definují další stav. ...který udělením ohodnocení určí, který z potomků odpovídá ohodnocení vzhledem k tajnému kódu. ...Zadavatel následně určí, který z potomků odpovídá ohodnocení vzhledem k tajnému kódu. 

% stavu, kterým je potomek $A$ vzhledem ke kódu $u$ a ohodnocení $r$. 
