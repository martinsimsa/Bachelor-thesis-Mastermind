...
\begin{dukaz}
    ...
    Pokud existuje nějaká cesta začínající i končící v $K_1$ vedoucí přes vrchol $K_2$, tak potom díky lemmatu \ref{lemmadisjunktnipotomci} platí $K_1 \subset K_2 \subset K_1$, což je spor. Tedy neexistuje žádná nekonstantní cesta začínající i končící ve stejném vrcholu a multigraf [n,k]-Mastermindu je orientovaný acyklický.
    
    
    
    % Pokud z vrcholu $K_1$ vede cesta do vrcholu $K_2$ a $K_2 \neq K_1$, tak 
    % $K_2 \subset K_1$ díky lemmatu \ref{lemmadisjunktnipotomci}. 
    % %Pokud $K_2 \neq K_1$, potom $K_2 \subset K_1$. 
    % Pokud existuje nějaká cesta z $K_1$ přes $K_2$ zpět do $K_1$, potom $K_2 \subset K_1 \subset K_2$, což je spor. Neexistuje tedy nekonstantní orientovaná cesta v [n,k]-Mastermindu a multigraf [n,k]-Mastermindu je tedy orientovaný acyklický.
\end{dukaz}
...
\begin{definice}[Rozvinutí orientovaného acyklického multigrafu]
    ...
    Množinu vrcholů $V$ je množina cest z kořene multigrafu $G$.
...
\begin{definice}[Deterministický algoritmus]\label{defobecnyalg}
    Deterministický algoritmus s prostorem valuací $\mathcal{F}$ a strategií $F$ je definovaný jako funkce, která tajnému kódu $v$ přiřadí posloupnost zahraných kódů s ohodnoceními $\textsc{Solve}[n, k, \mathcal{F}, F](v)$, podle předpisu algoritmu \ref{alg-default}.
