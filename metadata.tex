...
\def\Abstract{Práce se zabývá popisem algoritmů řešících hru Mastermind pro libovolný počet pozic a barev. Druhá kapitola popisuje reprezentaci stavu hry jako posloupnost pokusů s ohodnocením či jako množina zbývajících možností na tajný kód. Oba způsoby jsou reprezentovány grafem a je zde popsán vztah mezi těmito dvěma grafy. Dále je sestrojen obecný algoritmus sloužící k popisu konkrétních algoritmů řešící hru Mastermind. Ve třetí kapitole jsou popsány tři deterministické algoritmy, které hru Mastermind řeší. Jde o algoritmy Min-max, Max entropy a Most parts. Nakonec jsou tyto metody implementovány a otestovány na hře Mastermind o čtyřech pozicích a šesti barvách.}

% Anglická verze abstraktu
\def\AbstractEN{This thesis describes algorithms solving the game Mastermind for arbitrary number of positions and colours. Second chapter describes the state of the game in two ways, as a sequence of codes with evaluations and as a set of codes which could be the secret code.  Both sets of state representations are described in a graph and we show the relation between these graphs. Next, we construct a general algorithm solving Mastermind which we use for describing particular methods. In the third chapter, three deterministic algorithms are described. They are called Min-max, Max-entropy and Most parts. Finally, we implement these algorithms and show their results in the game Mastermind with four positions and six colours.}
